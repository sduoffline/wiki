...
- 调幅（ASK）：用载波的两个不同振幅表示0和1。
- 调频（FSK）：用载波的两个不同频率表示0（1.2KHz）和1（2.4KHz）。
- 调相（PSK）：用载波的起始相位的变化表示0 (同相）和1（反相）。
...
- 波分复用（WDM，Wave Division Multiplexing）：将整个波长频带被划分为若干个波长范围，每个用户占用一个波长范围来进行传输。适用于光纤传输。
  ...
  ![FDM示意图](https://s2.loli.net/2024/01/05/HMLv3SwRNhWxKy9.png)
...
数据链路层从网络层获取数据包，将其封装为“**帧头+有效载荷+帧尾**”的帧，数据链路层的核心就是帧的管理。
...
所以说，介质访问控制（MAC）是**当多个使用者共享同一个信道时，确定下一个使用者的问题**。
...
传输协议的要素包括寻址、建立连接三次握手、释放连接三次握手、流控制和缓冲。
