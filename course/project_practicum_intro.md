...
- 服务端：通常以HTTP（不通常的包括WebSocket和SSE）协议的方式提供接口，供（宽泛的）前端进行调用。服务端需要维护各项数据，处理业务逻辑，是整个项目中功能的核心。
- Web前端：供用户在浏览器中访问使用。
- 移动客户端：供用户在智能手机中使用。
- AI推理：使用某些框架（PyTorch、TensorFlow等）开发神经网络并部署（直接部署、ONNX Runtime等）并对外提供服务（HTTP、MQ等），也就是提供AI绘图功能的部分。
...
- Java：万金油，绝大多数同学最熟悉的语言，生态非常好，首选。
- Python：如果对Java生态（Spring Boot）等，可以考虑使用Python，优点是简单、框架多，但是维护成本和风险会高很多，除非真不会Java否则不建议。
- JavaScript/TypeScript：可以使用Nodejs开发服务端，只建议会点儿js，对Java/Pytohn完全不会的同学选择，问题和Python一样。
- Go：写Web可能比较别扭，但很轻量很快，只建议会的同学选择。
	> 为什么Go排在最后？因为：绝大多数人都不会。不建议贸然选择一个不熟悉的技术。

注意，选择语言并没多大意义，最重要的是**语言所绑定的生态**，先为各位介绍服务端还会涉及哪些内容：
...
- MySQL：不会有问题，选不会错。
- PostgreSQL：性能好，特性多，SQL方言与MySQL差异不大，可以图一乐。

简单来看，这俩选哪个都行，没什么坑。

如果你希望来点花活儿，还有很多NoSQL供你选择，按照使用频率降序如下：

- Redis：完全基于内存的键值对数据库，可以用于实现缓存、Session存储、分布式锁等很多功能。用上它，至少可以用于缓存吹一吹。
- MongoDB：难用的要死，真要存json可以考虑使用PostgreSQL。
- Neo4J：图数据库，如果你的选题涉及社交网络/知识图谱可以考虑用上，难度不高。
- Elastic Search：一款基于Document的数据库（或者说，搜索引擎），适合做搜索，大概率用不到，除非你希望实现复杂的全文搜索/推荐系统。
...
- GPT：无需多言，仅仅针对Prompt进行调整就可以做很多工作。
...
- UniLSeg：字节跳动和深研院合作推出的多模态网络，也十分牛皮。
...
- 对外暴露接口：通常使用Flask/FastAPI暴露HTTP接口就可以。
- 保持模型加载到显存后不会释放，避免重复加载模型。

至于怎么做。。择日细说吧，我们还有更重要的事情：要在端侧运行神经网络该怎么办？

首先，为各位介绍一些直接依靠框架运行神经网络以外的方法：

- ONNX Runtime：ONNX是一个开源方案，可以将神经网络统一为ONNX格式，并在任何提供ONNX Runtime的设备上运行神经网络。
- ncnn/mnn/tnn：一系列将神经网络编译到二进制文件的技术，更多用于在端侧进行部署。
